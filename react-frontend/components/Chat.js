...
function Chat({ socket, connectedUser, className }) {
    ...
    return (
        <div className={className}>
            ...
            <div className="message-list pb-4 flex flex-col overflow-y-auto bg-white h-full px-4 max-h-[80vh]">
                {
                    messages && messages.map((message, index) => {
                        ...
                        return (
                            <div ref={isLastMessage ? lastMessageRef : null} className={`${connectedUser.uuid === message.uuid ? 'self-end' : 'self-start'} ${(!lastUser || lastUser.uuid !== message.uuid) ? 'mt-3' : 'mt-1'} max-w-[80%]`} key={index}>
                                ...
                                <div className='px-4 py-2 bg-gradient-to-br from-sky-500 to-sky-400/80 text-white rounded-xl whitespace-pre-wrap'>
                                    {message.message}
                                </div>
                            </div>
                        )
